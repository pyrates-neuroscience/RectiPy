...
class Network(Module):
    ...
    def __getitem__(self, item: int):
        # TODO: Re-enable possibility to retrieve variables from rnn layers via names.
    ...
    def add_rnn_layer_from_yaml(self, node: Union[str, NodeTemplate], input_var: str, output_var: str,
                                ...
                                **kwargs) -> RNNLayer:
        """Adds an RNN layer to the `Network` instance.
        ...
        if spike_var is None and spike_def is None:
            rnn_layer = RNNLayer.from_yaml(node, var_dict['in_ext'], var_dict['out'], weights=weights,
            ...
            rnn_layer = SRNNLayer.from_yaml(node, var_dict['in_ext'], var_dict['out'], weights=weights,
        ...
        # add layer
        return self._add_layer(rnn_layer)

    def add_rnn_layer_from_template(self, template: CircuitTemplate, input_var: str, output_var: str,
                                    spike_var: str = None, spike_def: str = None, op: str = None,
                                    train_params: list = None, **kwargs) -> RNNLayer:
        """Add RNN layer to the `Network` instance from a YAML template.
    ...
    def add_ffwd_layer(self, n: int, weights: np.ndarray = None, train: Optional[str] = None,
        ...
        kwargs.update({"n_in": self.n, "n_out": n, "weights": weights, "dtype": dtype})
        if n == self.n and weights is None:
            output_layer = None
        elif train is None:
        ...
        # add layer
        return self._add_layer(output_layer.to(self.device))

    def remove_layer(self, idx: int = -1) -> Union[Linear, RNNLayer]:
    ...
    def fit_rls(self, inputs: np.ndarray, targets: np.ndarray, feedback_weights: np.ndarray = None,
        ...
        if self.output_layer is None:
            self.add_ffwd_layer(n=targets.shape[1], train="rls", **kwargs)
    ...
    def _add_layer(self, layer: Union[Linear, LayerStack, RNNLayer]) -> Union[Linear, LayerStack, RNNLayer]:
